...
/// MP3 解码器（FFmpeg backend 为主）。
///
/// - `not(feature="ffmpeg")`：占位实现
/// - `feature="ffmpeg")`：libavcodec
...
impl AudioDecoder for Mp3Decoder {
    ...
    fn send_packet(&mut self, packet: Option<CodecPacket>) -> CodecResult<()> {
        ...
        Err(CodecError::Unsupported("MP3 decoder requires ffmpeg feature"))
    }
...
mod ffmpeg_backend {
    ...
    impl Mp3Decoder {
        pub fn new() -> CodecResult<Self> {
            unsafe {
                let name = CString::new("mp3").map_err(|_| CodecError::InvalidData("codec name contains NUL"))?;
                let codec = ff::avcodec_find_decoder_by_name(name.as_ptr());
                if codec.is_null() {
                    return Err(CodecError::Unsupported("FFmpeg MP3 decoder not found"));
                }
...
#[cfg(feature = "ffmpeg")]
pub use ffmpeg_backend::Mp3Decoder;

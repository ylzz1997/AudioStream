...
class ReduceSum:
    def __init__(self, weight: Optional[list[float]] = None) -> None: ...
    def __call__(self, items: list[NodeBuffer]) -> NodeBuffer: ...
...
class ParallelAudioWriter(AudioSink):
    ...
    def __init__(self, writers: list[AudioFileWriter | LineAudioWriter]) -> None: ...

    def bind(self, writer: AudioFileWriter | LineAudioWriter) -> None: ...
...
class LineAudioWriter(AudioSink):
    """
    线性链路写端：processors(PCM->PCM)* -> writer。

    注意：传入的 writer 与 processors 会被“move/搬空”，之后不可再使用。
    """

    def __init__(self, writer: AudioFileWriter, processors: Optional[list[Processor]] = None) -> None: ...
